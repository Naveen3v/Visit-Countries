...
const filterList = initialCountriesList.filter(each => each.isVisited === true)

class App extends Component {
  state = {isVisited: true, visitList: filterList}

  addCountry = id => {
    const updatedList = initialCountriesList.filter(each => each.id === id)

    this.setState(each => ({
      visitList: [...each.visitList, ...updatedList],
    }))
  }

  removeCountry = id => {
    const remove = filterList.filter(each => each.id !== id)
    this.setState(each => ({
      visitList: [...remove],
    }))
  }
